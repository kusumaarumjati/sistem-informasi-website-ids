...
@section('title', 'Tambah Data customer 2')
            ...
            <div class="row">
              <div class="col-md-12">
                <div class="form-group">
                   <form method="POST" action="{{url('/tambahcust2/simpan/')}}">
                    {{ csrf_field() }}
                    <div class="form-group">
                ...
                <input type="hidden" name="filepath" class="image-tag">
                </div>
                <div class="col-md-6">
...
$(document).ready(function(){
  ...
  $('#province-a').on('change', function(){
    ...
    $.ajax({
      ...
      success:function(data){
       var a1=$('#city-a').html(data);
        for (var i=0; i<data.length; i++){
       a1.append('<option value="'+data[i].city_id+'">'+data[i].city_name+'</option>');
        }
        }
  ...
  $('#city-a').on('change', function(){
    ...
    $.ajax({
      ...
      //data:'prov_id='+prov_id,
      success:function(data){
          var a2=$('#district-a').html(data);
        for (var i=0; i<data.length; i++){
         a2.append('<option value="'+data[i].dis_id+'">'+data[i].dis_name+'</option>');
        }
        }
  ...
  $('#district-a').on('change', function(){
    ...
    $.ajax({
      ...
      //data:'prov_id='+prov_id,
      success:function(data){
          var a3=$('#subdis-a').html(data);
        for (var i=0; i<data.length; i++){
         a3.append('<option value="'+data[i].subdis_id+'">'+data[i].subdis_name+'</option>');
        }
        }
